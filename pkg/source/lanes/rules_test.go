...
func TestDeriveLane(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:         "operator provision step maps to provision",
			environment:  "dev",
			artifactPath: "https://example.com/run/artifacts/junit_operator.xml",
			testSuite:    "step graph",
			testName:     "aro-hcp-provision-environment",
			wantLane:     LaneProvision,
		},
		{
			name:         "operator test-local step maps to e2e",
			environment:  "dev",
			artifactPath: "artifacts/junit_operator.xml",
			testSuite:    "step graph",
			testName:     "aro-hcp-test-local",
			wantLane:     LaneE2E,
		},
		{
			name:         "operator alerts gather step maps to alert",
			environment:  "dev",
			artifactPath: "artifacts/junit_operator.xml",
			testSuite:    "step graph",
		...
		{
			name:         "operator build step maps to unknown (other bucket)",
			environment:  "dev",
			artifactPath: "artifacts/junit_operator.xml",
			testSuite:    "step graph",
			testName:     "aro-hcp-gather-snapshot",
			wantLane:     LaneUnknown,
		},
		{
